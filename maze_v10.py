...
class MazeEnv(gym.Env):
        ...
        if (0 <= next_pos[0] < self.num_rows) and (0 <= next_pos[1] < self.num_cols):
            ...
            if self.maze[next_pos[0], next_pos[1]] == '1':
                ...
                if next_pos == self.sub_goal_pos and self.sub_goal_pos not in self.reached_goals:
                    reward = 50  # Reward for reaching the sub-goal
    ...
    def get_q_value_color(self, q_value):
        ...
        normalized_q = (q_value - np.min(self.q_table_q)) / (np.max(self.q_table_q) - np.min(self.q_table_q) + 1e-5)
        ...
        for row in range(self.num_rows):
            for col in range(self.num_cols):
                ...
                if self.maze[row, col] == '1':
                    ...
                    max_q_value = np.max(self.q_table_q[row, col])
    ...
    def choose_action(self, state, q_table):
        if np.random.uniform(0, 1) < self.epsilon:
            ...
            row, col = state
            return np.argmax(q_table[row, col])  # Exploit the best-known action

    def update_q_value_qlearning(self, state, action, reward, next_state):
        row, col = state
        next_row, next_col = next_state
        best_next_action = np.argmax(self.q_table_q[next_row, next_col])
        td_target = reward + self.gamma * self.q_table_q[next_row, next_col, best_next_action]
        td_error = td_target - self.q_table_q[row, col, action]
        self.q_table_q[row, col, action] += self.alpha * td_error

    def update_q_value_sarsa(self, state, action, reward, next_state, next_action):
        row, col = state
        next_row, next_col = next_state
        td_target = reward + self.gamma * self.q_table_sarsa[next_row, next_col, next_action]
    ...
    def visualize_learned_path(self):
        ...
        while not done:
            ...
            row, col = state
            action = np.argmax(self.q_table_q[row, col])
